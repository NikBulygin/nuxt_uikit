<template>
    <div class="table-wrapper">
        <!-- Toolbar -->
        <div class="toolbar mb-2 h-[44px] flex items-center justify-between gap-2">
            <!-- Left side -->
        ...
        <div class="table-container w-full">
            <div class="table-grid" :style="gridTemplateColumns">
                <!-- Checkbox header -->
                <div v-if="isEditMode" class="table-header-cell p-2 bg-primary-500 text-text-primary dark:bg-primary-500 dark:text-text-primary flex items-center">
                    <input 
                        type="checkbox" 
                        v-model="selectAll"
                        ...
                        <div v-if="column.type !== 'formula' && !isEditMode" class="flex items-center">
                            <svg 
                                ...
                                :class="{
                                    ...
                                    'rotate-180': props.sorted?.key === column.key && props.sorted.direction === 'desc'
                                }"
                                viewBox="0 0 24 24" 
                                fill="none" 
                                stroke="currentColor"
                ...
                <template v-for="(row, rowIndex) in displayData" :key="rowIndex">
                    ...
                    <div 
                        ...
                        :class="[
                            isNewRow(rowIndex) ? 'bg-gray-700 text-white' : (rowIndex % 2 === 0 ? 'bg-primary-50 dark:bg-primary-400' : 'bg-primary-100 dark:bg-primary-500')
                        ]"
                    >
                        <input 
                            type="checkbox" 
                            ...
                            class="w-4 h-4 rounded border-gray-300 cursor-pointer"
                            :class="{ 'bg-gray-600 border-gray-500': isNewRow(rowIndex) }"
                        />
                    </div>

                    <!-- Data cells -->
                    <template v-for="column in props.meta.columns" :key="`${rowIndex}-${column.key}`">
                        <div 
                            ...
                            :class="[
                                ...
                                editedDataErrors[rowIndex]?.[column.key] && 'border-red-500 border'
                            ]"
                        >
                            <template v-if="isEditMode && column.type !== 'formula'">
                                <InputText
                                    ...
                                    @error="(msg) => handleValidationError(rowIndex, column.key, msg)"
                            ...
                            <template v-else>
                                {{ column.type === 'formula' ? column.formula(getRowData(rowIndex)) : getRowData(rowIndex)[column.key] }}
                            </template>
                        </div>
                    </template>
                </template>
...
interface ValidationMeta {
    ...
    type?: 'email' | 'strongPassword' | 'basicPassword' | 'numbersOnly' | 'lettersOnly' | 'url' | 'phone' | 'custom'
    step?: number
    precision?: number
    format?: string
}
...
const emit = defineEmits(['copy', 'delete', 'update:itemsPerPage', 'update:page', 'save', 'sort', 'add'])
...
const hasSelected = computed(() => Object.values(selectedRows.value).some(v => v))
...
const getRowData = (index: number) => {
    ...
    props.meta.columns.forEach(column => {
        if (column.type === 'date') {
            processedData[column.key] = processedData[column.key] ? new Date(processedData[column.key]) : null
        }
    })
...
const initEditedData = () => {
    editedData.value = props.data.reduce((acc, row, index) => {
        ...
        props.meta.columns.forEach(column => {
            if (column.type === 'date') {
                acc[index][column.key] = acc[index][column.key] ? new Date(acc[index][column.key]) : null
            }
        })
        return acc
...
const copySelected = () => {
    ...
    const selectedData = selectedIndices.map(index => {
        ...
        const sourceData = isNewRow(index) ? editedData.value[index] : props.data[index]
        ...
        props.meta.columns.forEach(column => {
            if (column.type === 'date' && rowData[column.key]) {
                ...
                rowData[column.key] = rowData[column.key] instanceof Date 
...
const getValidationPattern = (validation?: ValidationMeta): RegExp | null => {
